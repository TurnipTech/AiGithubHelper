...
export class AIProviderFactory {
  ...
  private static async getFallbackProvider(
    requestedProvider: 'claude' | 'gemini',
  ): Promise<AIProvider> {
    const fallbackProvider =
      requestedProvider === 'claude' ? this.geminiProvider : this.claudeProvider;
    ...
    throw new Error(
      `Neither ${requestedProvider} nor fallback provider are available. Please ensure at least one AI CLI is installed and configured.`,
    );
  }
  ...
  private static async selectBestAvailableProvider(): Promise<AIProvider> {
    ...
    throw new Error(
      'No AI providers are available. Please ensure Claude CLI or Gemini CLI is installed and configured.',
    );
  }
